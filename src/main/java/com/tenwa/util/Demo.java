...
public class Demo {
    ...
    public static boolean isNotLaunchPayInstruction(String flowId, String planId) throws Exception {
        List<Map<String, String>> dataBySql = OperateMapUtil.getDataBySql("select count(1) as num from PAY_INSTRUCTION where FLOWUNID = ? and PLAN_ID = ? and nvl(INSTRUCTIONSTATUSCOD,0)  = 1 ", flowId, planId);
